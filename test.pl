...
my $cgi = new CGI;
my $filename = 'berkeleydb.db';

my $session = new CGI::Session::BerkeleyDB(undef, {FileName=>$filename}) or die $CGI::Session::errstr;
...
$session = new CGI::Session::BerkeleyDB($sid, {FileName=>$filename});
